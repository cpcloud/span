...
def compute_threshold(sp, fs, contact, sc=5.0, const=0.6745, indfact=10):
    ...
    Returns
    -------
    thresholded_data : array
    """
    samps = sp['data'][contact, :][:(indfact * fs)]
    med_abs_samps = np.median(np.abs(samps))
    return sc * med_abs_samps / const
...
def main(filename, contact, window, thresh, shank, prefix, dtype='h'):
    ...
    try:
        ...
        for i, elecs in enumerate(elsd, start=1):
            ...
            for func, arg_set in zip(funcs, arg_sets):
...
def parse_args():
    ...
    for n, t, d, s, h in zip(names, types, defaults, short_names,
                                        help_strings):
